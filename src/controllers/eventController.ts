...
import { createNewEventForGroup, getAllEventsForAUser } from '../services/EventService.js';
...
const createNewEvent = async (req: Request, res: Response, next: NextFunction): Promise<void | undefined> => {
    const { groupId, ...eventToCreate } = req.body;
    try {
        const event = await createNewEventForGroup(eventToCreate, Number(req.user.userId), Number(groupId));
